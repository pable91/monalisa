...
class UserApiTest {
    ...
    void signupTest() throws Exception {
        ...
        UserResponseDto.SignUp responseDto = UserResponseDto.SignUp.of(
            User.createUser("kim", "1234", "kim", "kim@naver.com"));

        given(userService.signup(any(UserRequestDto.SignUp.class)))
            .willReturn(responseDto);

        // when, then
        mockMvc.perform(post(host + "/user/signup").with(csrf())
                .accept(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(requestDto))
                .contentType(MediaType.APPLICATION_JSON)
            )
            .andExpect(status().isOk())
            .andDo(document(host + "/user/signup",
    ...
    void loginTest() throws Exception {
        ...
        // when, then
        mockMvc.perform(post(host + "/user/login").with(csrf())
                .accept(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(requestDto))
                .contentType(MediaType.APPLICATION_JSON)
            )
            ...
            .andDo(document(host + "/user/login",
                Preprocessors.preprocessRequest(prettyPrint()),
                Preprocessors.preprocessResponse(prettyPrint()),
                requestFields(
                    fieldWithPath("accountId").description("사용자 아이디").type(JsonFieldType.STRING),
    ...
    void profileTest() throws Exception {
        UserResponseDto.Profile responseDto = UserResponseDto.Profile.of(
            User.createUser("kim", "1234", "kim", "kim@naver.com"));

        given(userService.profile(any()))
            .willReturn(responseDto);

        mockMvc.perform(get(host + "/user/profile")
            )
            .andExpect(status().isOk())
            .andDo(document(host + "/user/profile",
                Preprocessors.preprocessRequest(prettyPrint()),
                Preprocessors.preprocessResponse(prettyPrint()),
                responseFields(
    ...
    void findMyOrderListTest() throws Exception {
        ...
        mockMvc.perform(get(host + "/user/orderList")
            )
            .andExpect(status().isOk())
            .andDo(document(host + "/user/orderList",
                Preprocessors.preprocessRequest(prettyPrint()),
                Preprocessors.preprocessResponse(prettyPrint()),
                responseFields(
                    fieldWithPath("orderList").type(JsonFieldType.ARRAY).description("주문 리스트"),
                    fieldWithPath("orderList[].totalPrice").type(JsonFieldType.NUMBER)
                        .description("아이디"),
                    fieldWithPath("orderList[].orderDate").type(JsonFieldType.STRING)
                        .description("날짜"),
                    fieldWithPath("orderList[].bookList[].userName").type(JsonFieldType.STRING)
                        .description("아이디"),
                    fieldWithPath("orderList[].bookList[].name").type(JsonFieldType.STRING)
                        .description("책 이름"),
                    fieldWithPath("orderList[].bookList[].desc").type(JsonFieldType.STRING)
                        .description("책 설명"),
                    fieldWithPath("orderList[].bookList[].cost").type(JsonFieldType.NUMBER)
                        .description("책 가격"),
                    fieldWithPath("orderList[].bookList[].author").type(JsonFieldType.STRING)
                        .description("저자"),
                    fieldWithPath("orderList[].bookList[].isSold").type(JsonFieldType.BOOLEAN)
                        .description("판매여부"),
                    fieldWithPath("orderList[].bookList[].like").type(JsonFieldType.NUMBER)
                        .description("좋아요 개수")
                )
            ));
    }
